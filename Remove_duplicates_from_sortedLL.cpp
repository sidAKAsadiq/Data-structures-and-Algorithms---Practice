...
/*2nd psbl condition that can be used in while -> kind of jugaar
    ...
    void remove_duplicates_from_sorted_LL(Node* &head){
        Node* checker = head;
        Node* ahead = head->address;
        Node* deleter;
        while(ahead != NULL){
            cout<<"\nChecker : "<<checker->data<<endl;
            cout<<"ahead : "<<ahead->data<<endl;
            if(checker->data != ahead->data){
                checker->address = ahead;
                checker = ahead;
                ahead = ahead->address;    
            }
            else{   
                checker->address = NULL;
                deleter = ahead;    //deletes the duplicate nodes
                ahead = ahead->address;
                delete deleter;
            }
...
int main() {
    ...
    head->remove_duplicates_from_sorted_LL(head);

...
class singly_linked_list{
    ...
    void sort_even_before_odd_by_rec(int nodes , int &counter , int &index , Node* temp){
        //Base Case
        if(nodes == counter){
            return;
        }
        ...
        counter++;
        sort_even_before_odd_by_rec(nodes,counter,index,temp);
    } 
...
main(){
    ...
    int i = 1;
    Node* temp = s1.head;
    s1.sort_even_before_odd_by_rec(s1.count_number_of_nodes(),c,i,temp);
    s1.traverse_LL();
